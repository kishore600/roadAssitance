...
router.get('/saved-locations', async (req: AuthenticatedRequest, res: Response) => {
    try {
...
// Save new location
router.post('/saved-locations', async (req: AuthenticatedRequest, res: Response) => {
...
router.delete('/saved-locations/:id', async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
    ...
    try {
        ...
        if (location?.is_default) {
            const { data: remainingLocations, error: remainingError } = await supabaseAdmin
                .from('saved_locations')
                .select('id')
                .eq('customer_id', userId)
                .limit(1);
                
            if (!remainingError && remainingLocations && remainingLocations.length > 0) {
                await supabaseAdmin
                    .from('saved_locations')
...
router.patch('/saved-locations/:id/set-default', async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.get('/saved-locations/default', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
// Update location
router.put('/saved-locations/:id', async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
